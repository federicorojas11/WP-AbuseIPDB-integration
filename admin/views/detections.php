    ...
    <div class="aipdb-detections-filters">
        <div class="aipdb-filter-row">
            <div class="aipdb-filter-group">
                ...
                <input type="text" id="detection-search" placeholder="<?php esc_attr_e('IP address, User Agent, URI...', 'wp-abuseipdb-integration'); ?>" />
            </div>
                ...
                <?php if (!empty($detections['data'])): ?>
                    <?php foreach ($detections['data'] as $detection): ?>
                        <tr data-id="<?php echo intval($detection->id); ?>">
                            ...
                            <td>
                                <?php echo esc_html(mysql2date(__('M j, Y g:i A', 'wp-abuseipdb-integration'), $detection->created_at)); ?>
                            </td>
                            <td>
                                <strong><?php echo esc_html($detection->ip_address); ?></strong>
                                <div class="row-actions">
                                    <span><a href="https://www.abuseipdb.com/check/<?php echo urlencode($detection->ip_address); ?>" target="_blank" rel="noopener noreferrer"><?php _e('Check AbuseIPDB', 'wp-abuseipdb-integration'); ?></a></span>
                                </div>
                            </td>
                            <td>
